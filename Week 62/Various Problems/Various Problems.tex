...
\section*{Problem 10}
    ...
    (b) Pick two events A and B that would be connected by a signal that travels at twice the speed of light and pick A to have happened first and list them.

    (c) What is the relative velocity of a frame where observers in that frame would see A and B happen at the same time?

    (d) What are the relative velocities of frames that would see B happen first?
    ...
    \subsection*{Solution (a)}
        ...
        \begin{gather*}
            ...
            \frac{t_B - (u/c^2)x_B}{\sqrt{1 - u^2/c^2}} \leq \frac{t_A - (u/c^2)x_A}{\sqrt{1 - u^2/c^2}}\\
            t_B - (u/c^2)x_B \leq t_A - (u/c^2)x_A
        \end{gather*}
    \pagebreak

\section*{Problem 11}
    Consider two protons headed towards each other with the same speeds. Each proton has mass $m_p = 938.8\,\unit{\eV/\light^2}$. 
