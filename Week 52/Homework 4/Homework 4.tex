...
\begin{document}
    ...
    \section{Problem 5}
        ...
        \subsection{Solution}
            ...
            Also update Equation (\ref{5.0}) to cover twice the area ($\e{-ax^2}$ is symmetric about the y-axis).
            \begin{align}
                \int_{-\infty}^{\infty} \e{-\frac{m\omega x^2}{\hbar}}\,dx  
                    &=  \int_{0}^{\infty} \e{-\frac{m\omega x^2}{\hbar}}\,dx + \int_{-\infty}^{0} \e{-\frac{m\omega x^2}{\hbar}}\,dx\\
                    &=  \int_{0}^{\infty} \e{-\frac{m\omega x^2}{\hbar}}\,dx + \int_{0}^{\infty} \e{-\frac{m\omega x^2}{\hbar}}\,dx\\
                    &=  2 \int_{0}^{\infty} \e{-\frac{m\omega x^2}{\hbar}}\,dx
                    =   \sqrt{\frac{\pi}{a}}
            \end{align}

            With $a = \frac{m\omega}{\hbar}$, we can use ($\ref{5.0}$).
            \begin{gather}
                1   =   A^2 * \sqrt{\frac{\pi}{\frac{m\omega}{\hbar}}}
                    =   A^2 \sqrt{\frac{\pi\hbar}{m\omega}}\\
                A^2 =   \sqrt{\frac{m\omega}{\pi\hbar}} \to
                \boxed{A = \sqrt[4]{\frac{m\omega}{\pi\hbar}}}
            \end{gather}
    ...
    \section{Problem 7}
        \begin{wrapfigure}{r}{0.33\textwidth}
            \includegraphics[width=0.9\linewidth]{Images/Fig 2-7.png}
            \caption[2.7]{Potential Well}
            \label{fig:2.7}
        \end{wrapfigure}
        (a) For a particle moving in the potential well shown in Figure \ref{fig:2.7}, write down the Schrödinger equations for the region where $0 \leq x \leq L$ and the region where $x \geq L$. 
        ...
        \subsection{Solution (a)}
            Recall Schrödinger's time-independent equation.
            \begin{equation}
                \label{eq:71} \tag{71}
                -\frac{\hbar^2}{2m}\,\diffp[2]{\psi}{x} + V(x)\,\psi = E\,\psi
            \end{equation}
            
            This can be applied for both cases of the value of $V(x)$.

            \subsubsection{$0 \leq x \leq L$}
                Within this boundary, $V = 0$.
                We can plug it into (\ref{eq:71}).
                \begin{gather}
                    \boxed{-\frac{\hbar^2}{2m}\,\diffp[2]{\psi}{x} = E\,\psi}
                \end{gather}

            \subsubsection{$x \geq L$}
                Do the same for $V = V_0$.
                \begin{equation}
                    \boxed{-\frac{\hbar^2}{2m}\,\diffp[2]{\psi}{x} + V_0\,\psi = E\,\psi}
                \end{equation}

        \subsection{Solution (b)}
            This would also be separated into areas.
            I will be finding solutions using exponentials.

            \subsubsection{$0 \leq x \leq L$}
                Start with the equation we found in part (a).
                \begin{gather}
                    -\frac{\hbar^2}{2m}\,\diffp[2]{\psi}{x} = E\,\psi
                \end{gather}

                Solve for $\diffp[2]{\psi}{x}$.
                \begin{gather}
                    \diffp[2]{\psi}{x} = -\frac{2mE}{\hbar^2}\,\psi
                        =   \left( i \frac{\sqrt{2mE}}{\hbar} \right)^2\,\psi
                \end{gather}

                This can be placed into an exponential.
                \begin{align} \label{eq:7.b.3}
                    \boxed{\psi = A\e{i \frac{\sqrt{2mE}}{\hbar} x}}
                \end{align}

            \subsubsection{$x \geq L$}
                Start with the equation from part (a).
                \begin{equation}
                    -\frac{\hbar^2}{2m}\,\diffp[2]{\psi}{x} + V_0\,\psi = E\,\psi
                \end{equation}

                Collect all $\psi$ and solve for $\diffp[2]{\psi}{x}$.
                \begin{gather}
                    -\frac{\hbar^2}{2m}\,\diffp[2]{\psi}{x} = (E - V_0)\,\psi\\
                    \diffp[2]{\psi}{x} = -\frac{2m(E - V_0)}{\hbar^2}\,\psi
                        =   \frac{\sqrt{2m(V_0 - E)}}{\hbar}\,\psi
                \end{gather}

                This can be placed into an exponential.
                \begin{align}
                    \boxed{\psi = A\e{\frac{\sqrt{2m(V_0 - E)}}{\hbar} x}}
                \end{align}

                Note that this could be a complex exponential, dependant on whether $V_0 - E$ is positive or negative.
                It is equivalent to (\ref{eq:7.b.3}) when $V_0 = 0$.

        \subsection{Solution (c)}
            If the potential is infinite at $x = 0$, that means there is no chance in hell that the particle will be found there.
            We can put this as an inital condition.
            \begin{equation}\label{eq:7.c.ic0}
                \psi(0) = 0
            \end{equation}

            We can find a specific solution to $\psi$ with the initial condition of (\ref{eq:7.c.ic0}).
            \begin{align}
                \psi(0) = 0 &=  A\e{\frac{\sqrt{2m(V_0 - E)}}{\hbar} * 0} + c\\
                c   &=  -A\e{0} = -A\\
                & \boxed{\psi(x) = A\e{\frac{\sqrt{2m(V_0 - E)}}{\hbar} x} - A}
            \end{align}

            There appears to be no indication of what $\diffp{\psi}{x}$ is at $x = 0$.
        \pagebreak
    ...
    \section{Problem 8}
        ...
        \subsection{Solution}
            \begin{proof}[Show]
                Take the first derivative of (\ref{2.41}) with respect to $t$, and the second derivative with respect to $x$.
                \begin{align}
                    \diffp{\psi}{t} &=  -i\omega Ae^{ikx} \cdot e^{-i\omega t}
                        =   -i\omega\psi\\
                    \diffp[2]{\psi}{x}  &=  -k^2 Ae^{ikx} \cdot e^{-i\omega t}
                        =   -k^2 \psi
                \end{align}
    
                We can plug these into (\ref{2.47}).
                \begin{gather}
                    \frac{-\hbar^2}{2m}\,(-k^2) \psi + V(x,t) \psi = i \hbar (-i\omega) \psi\\
                    \frac{\hbar^2 k^2}{2m} + V(x,t) = \hbar\omega \label{8.4}
                \end{gather}
    
                Recall a handful of equations involving both values in (\ref{8.4}) and $2\pi$.
                \begin{multicols}{3}
                    \begin{equation}
                        \hbar = \frac{h}{2\pi}
                    \end{equation}
                ...
                We can plug all of these into (\ref{8.4}).
                \begin{gather}
                    \frac{\left( \frac{h}{2\pi} \right)^2 \left( \frac{2\pi}{\lambda} \right)^2}{2m} + V(x,t) = \frac{h}{2\pi} * 2\pi f\\ \notag
                    \frac{\left( \frac{h}{\cancel{2\pi}} \right)^2 \left( \frac{\cancel{2\pi}}{\lambda} \right)^2}{2m} + V(x,t) = \frac{h}{\cancel{2\pi}} * \cancel{2\pi} f\\
                    \frac{h^2}{2m\lambda^2} + V(x,t) = hf \label{8.8}
                \end{gather}
    
                To proceed, we recall Planck's relation and De Broglie's relation.
                \begin{gather}
                    E = hf\\
                    \lambda = \frac{h}{p} \to p = \frac{h}{\lambda}
                \end{gather}
    
                These can be plugged into (\ref{8.8}).
                \begin{gather}
                    \frac{p^2}{2m} + V(x,t) = E\\
                    K + V(x,t) = E
                \end{gather}
    
                Unless there's a new kind of energy that is not potential or kinetic, this we know is true.
            \end{proof}
        \pagebreak
    ...
    \section{Problem 9}
        ...
        \subsection{Solution}
            ...
            There exists an equation that can convert us from the time-dependant to the time-independent equation involving the energy.
            \begin{equation}
                \psi(x,t) = u_E(x)\e{-i\frac{E}{\hbar}t}
            \end{equation}

            This can be used with the equation for energy from quantum numbers.
            \begin{gather}
                E = \frac{n^2 \pi^2 \hbar^2}{2mL^2}\\
                \frac{E}{\hbar} = \frac{n^2 \pi^2 \hbar}{2mL^2} = \frac{n^2 \pi h}{4mL^2}\\ 
                \psi(x,t,n) = u_E(x)\e{-i\frac{n^2 \pi h}{4mL^2}t} \label{9.6}
            \end{gather}

            We can plug in even values of $n \in \mathbb{N}$.
            \begin{align}
                \psi(x,t,2) &=  u_E(x)\e{-i\frac{4 \pi h}{4mL^2}t}  =   u_E(x)\e{-i\frac{\pi h}{mL^2}t}\\
                \psi(x,t,4) &=  u_E(x)\e{-i\frac{16 \pi h}{4mL^2}t} =   u_E(x)\e{-i\frac{4 \pi h}{mL^2}t}\\
                \psi(x,t,6) &=  u_E(x)\e{-i\frac{36 \pi h}{4mL^2}t} =   u_E(x)\e{-i\frac{9 \pi h}{mL^2}t}
            \end{align}

            We can next recall the solutions to even numbered wave functions from earlier in the chapter.
            \begin{equation}
                u_E (x) = \psi(x) = \sqrt{\frac{2}{L}} \cos\left( \frac{n\pi x}{L} \right)
            \end{equation}

            Plug that into our above equation (\ref{9.6}). 
            \begin{gather}
                \psi(x,t,n) = \sqrt{\frac{2}{L}} \cos\left( \frac{n\pi x}{L} \right) \e{-i\frac{n^2 \pi h}{4mL^2}t}
            \end{gather}

            This is pretty much a solution to it and how it would evolve.
